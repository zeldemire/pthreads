	...
	*/
	...
	while(please_quit == 0) {
		if(((thread_info_t *)data)->string[0] > ((thread_info_t *)data)->string[1]) {
            workperformed = 1;
            info->swapcount++;
			tmp = ((thread_info_t *)data)->string[0];
			((thread_info_t *)data)->string[0] = ((thread_info_t *)data)->string[1];
			((thread_info_t *)data)->string[1] = tmp;
        }
	sched_yield();
	}
	return (void *)info;
...
int smp2_main(int argc, char **argv) {
	...
	strncpy(string,argv[1],MAX); // Why is this necessary? Why cant we give argv[1] directly to the thread functions?ss
